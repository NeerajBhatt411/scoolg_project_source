...
const MainLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-background font-body-md text-on-surface">
      ...
      <aside className="hidden lg:flex h-screen w-64 fixed left-0 top-0 bg-[#f7f9fb] border-r-[1.5px] border-[#e0e7ff] flex-col py-6 px-4 z-50">
        <div className="mb-10 px-2 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] font-bold text-secondary uppercase tracking-widest opacity-70">Teacher Portal</p>
          </div>
        </div>
        <p className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-widest px-4 mb-2">Menu</p>
        ...
        <div className="mt-auto pt-4 space-y-3">
          <div className="flex items-center gap-3 p-3 rounded-2xl bg-surface-container-low border border-surface-container">
            <img src={avatar} alt="" className="w-10 h-10 rounded-xl object-cover ring-1 ring-primary/15" />
            <div className="min-w-0 flex-1">
              <p className="text-label-md font-bold text-on-surface leading-tight truncate">{teacher?.fullName || 'Teacher'}</p>
              <p className="text-[11px] text-on-surface-variant">{teacher?.teacherAppId}</p>
            </div>
            <button onClick={handleLogout} title="Logout" className="w-9 h-9 rounded-xl bg-error/10 text-error flex items-center justify-center hover:bg-error/20 transition-all active:scale-90 shrink-0">
      ...
      {/* DESKTOP TOP BAR */}
      <header className="hidden lg:flex fixed top-0 left-0 w-full h-16 justify-between items-center pl-72 pr-8 bg-white/80 backdrop-blur-md border-b border-surface-container z-40">
        <div>
          <h2 className="text-title-lg font-bold text-on-surface leading-tight">{greeting}, {teacher?.fullName?.split(' ')[0] || 'Teacher'} 👋</h2>
          <p className="text-label-md text-on-surface-variant leading-none">{todayStr}</p>
        </div>
        <button className="w-10 h-10 rounded-2xl bg-surface-container-low hover:bg-surface-container border border-surface-container flex items-center justify-center text-on-surface-variant transition-colors relative">
      ...
      <header className="lg:hidden flex justify-between items-center px-container-margin w-full h-16 bg-surface shadow-sm sticky top-0 z-50">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 rounded-xl overflow-hidden bg-primary/10 flex items-center justify-center border border-surface-container shrink-0">
            {school?.logo
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] font-bold text-secondary uppercase tracking-widest opacity-70 mt-0.5">Teacher Portal</p>
          </div>
        </div>
        <button onClick={() => navigate('/profile')} className="shrink-0 active:scale-90 transition-transform">
          <img src={avatar} alt="Profile" className="w-10 h-10 rounded-full object-cover ring-2 ring-primary/15" />
        </button>
      </header>
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-50 bg-white border-t border-surface-container shadow-[0_-8px_26px_-14px_rgba(15,23,42,0.25)]">
            ...
            <NavLink key={item.path} to={item.path} className="flex-1">
              {({ isActive }) => (
                <div className="flex flex-col items-center justify-center gap-1 active:scale-90 transition-transform">
                  <div className={`flex items-center justify-center h-8 w-14 rounded-full transition-all duration-300 ${isActive ? 'bg-primary/12' : ''}`}>
                    <span className={`material-symbols-outlined text-[24px] ${isActive ? 'text-primary' : 'text-on-surface-variant'}`} style={isActive ? { fontVariationSettings: "'FILL' 1" } : {}}>{item.icon}</span>
                  </div>
                  <span className={`text-[10px] font-bold tracking-tight ${isActive ? 'text-primary' : 'text-on-surface-variant'}`}>{item.name}</span>
                </div>
              )}
            </NavLink>
          ))}
