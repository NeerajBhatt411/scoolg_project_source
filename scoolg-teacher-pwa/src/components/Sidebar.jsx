        ...
        <aside className={`w-[280px] h-screen fixed left-0 top-0 overflow-y-auto bg-[#f7f9fb] border-r-[1.5px] border-[#e0e7ff] flex flex-col pt-3 pb-6 px-4 z-50 transition-transform duration-300 lg:translate-x-0 ${mobileOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'}`}>
            <div className="flex items-center justify-between gap-2">
                <button onClick={() => { navigate('/dashboard'); onClose(); }} title={school?.name} className="mb-3 px-2 flex items-center justify-start gap-3 flex-1 min-w-0">
                    <div className="h-16 w-16 shrink-0 relative flex items-center justify-center overflow-hidden rounded-2xl bg-white border border-slate-100 shadow-sm">
                        {school?.logo ? (
                            <>
                            ...
                            <span className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white text-2xl font-black flex items-center justify-center shadow-sm">
                                {(school?.name || 'T').charAt(0).toUpperCase()}
                            </span>
                        )}
                    </div>
                    <p className="text-[15px] font-extrabold text-[#191c1e] leading-tight text-left">Teacher Portal</p>
                </button>
