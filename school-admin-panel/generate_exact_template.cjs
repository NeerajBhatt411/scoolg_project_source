...
async function generate() {
    ...
    // Style the header row EXACTLY like the app template
    ...
    headers.forEach((h, index) => {
        ...
        if (h.header.includes('(Required)')) {
            ...
            cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF64748B' } }; // Gray
        }
    });
    ...
    for (let i = 0; i < 20; i++) {
        ...
        // Add dropdowns
        sheet.getCell(`E${rowNum}`).dataValidation = { type: 'list', allowBlank: true, formulae: ['"Male,Female,Other"'] };
        sheet.getCell(`F${rowNum}`).dataValidation = { type: 'list', allowBlank: true, formulae: ['"A+,A-,B+,B-,O+,O-,AB+,AB-"'] };
    }

    await workbook.xlsx.writeFile('C:/Users/neera/Downloads/Exact_Template_20_Students.xlsx');
}
