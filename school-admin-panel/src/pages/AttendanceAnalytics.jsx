    ...
    return (
        <div className="p-4 sm:p-8 space-y-6 max-w-[1100px] mx-auto pb-20">
            ...
            {loading ? (
                ...
                // No real attendance recorded yet — show a clean empty state
                // instead of fake/sample charts.
                <div className="bg-white p-10 sm:p-16 rounded-3xl border border-dashed border-slate-200 shadow-sm flex flex-col items-center text-center">
                    <div className="w-24 h-24 rounded-[32px] bg-slate-50 flex items-center justify-center mb-6">
                        <span className="material-symbols-outlined text-5xl text-slate-200">bar_chart</span>
                    </div>
                    <h3 className="text-xl font-black text-slate-700 tracking-tight mb-1">No data found</h3>
                    <p className="text-sm text-slate-500 font-semibold max-w-xs">Mark attendance for your classes and the analytics will appear here.</p>
                    <button onClick={() => navigate('/attendance')} className="mt-6 px-6 py-3 bg-[#2563eb] text-white font-bold text-sm rounded-xl shadow-sm shadow-blue-500/30 hover:scale-95 transition-all">
                        Go to Attendance
                    </button>
                </div>
            ) : (
