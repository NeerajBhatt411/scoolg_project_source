...
/**
 * Change-password screen. Serves two modes:
 *  - forced: account still on the default password (isPasswordChanged === false).
 *            No current-password field, no escape (except sign out).
...
const ChangePassword = () => {
  ...
  const inputCls = 'flex h-[54px] w-full rounded-xl border border-slate-200 bg-slate-50 pl-12 pr-12 text-sm font-semibold transition-colors placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:bg-white text-slate-900';
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-white p-5 sm:p-8">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-[460px]">
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.06)] p-8 sm:p-10">
          ...
          {done ? (
            <>
              <h1 className="text-[24px] font-extrabold tracking-tight text-slate-900">All set!</h1>
              <p className="text-[15px] font-medium text-slate-500 mt-1.5">Your password has been updated. Taking you in…</p>
            </>
          ) : (
            <>
              <p className="text-[11px] font-extrabold uppercase tracking-[0.25em] text-blue-600 mb-2">
                {forced ? 'Secure your account' : 'Account security'}
              </p>
              <h1 className="text-[24px] font-extrabold tracking-tight text-slate-900 leading-tight">
                {forced ? 'Set a new password' : 'Change your password'}
              </h1>
              <p className="text-[15px] font-medium text-slate-500 mt-1.5 mb-6">
                {forced
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                <button disabled={loading} type="submit"
                  className="w-full !mt-6 bg-blue-600 hover:bg-blue-700 text-white rounded-xl flex items-center justify-center gap-2 transition-colors disabled:opacity-50 text-[15px] font-bold h-[54px] shadow-lg shadow-blue-600/20 active:scale-[0.98]">
              ...
              <div className="mt-5 text-center">
                {forced ? (
                  <button onClick={logout}
                    className="inline-flex items-center gap-1.5 text-sm text-slate-500 font-semibold hover:text-slate-700 transition-colors">
